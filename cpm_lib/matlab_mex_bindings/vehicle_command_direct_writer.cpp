...
private:
    ...
    std::shared_ptr<cpm::Writer<VehicleCommandDirectPubSubType>> writer;
public:
    /**
     ...
     * \param inputs Inputs given by the calling Matlab script. Here: The VehicleCommandDirect Matlab Object to send. Optional: Matlab Domain ID.
     */
    void operator()(matlab::mex::ArgumentList outputs, matlab::mex::ArgumentList inputs) {   
        ...
        if (!participant || !writer)
        {
            ...
            writer = std::make_shared<cpm::Writer<VehicleCommandDirectPubSubType>>(
    ...
    void checkArguments(matlab::mex::ArgumentList outputs, matlab::mex::ArgumentList inputs) {
        ...
        if (inputs.size() < 1 || inputs.size() > 2)
        {
            matlabPtr->feval(u"error", 0, 
                std::vector<matlab::data::Array>({ factory.createScalar("Wrong input size. Input must be an object of type VehicleCommandDirect, followed by an optional domain_id argument (default is 1)!") }));
        }

        //Test for correct class w.r.t. message to send
        std::vector<matlab::data::Array> args{ inputs[0], factory.createCharArray("VehicleCommandDirect") };
        matlab::data::TypedArray<bool> result = matlabPtr->feval(u"isa", args);
        if (result[0] != true) {
            matlabPtr->feval(u"error", 0, 
               std::vector<matlab::data::Array>({ factory.createScalar("Input must be an object of type VehicleCommandDirect!") }));
        } 
     ...
     */
    VehicleCommandDirect get_vehicle_command_from_input(matlab::mex::ArgumentList inputs)
    {
        matlab::data::ArrayFactory factory;

        VehicleCommandDirect command;
        ...
        //Get direct data
        matlab::data::TypedArray<double> throttle_ml = matlabPtr->getProperty(object, u"motor_throttle");
        matlab::data::TypedArray<double> steering_ml = matlabPtr->getProperty(object, u"steering_servo");
        command.motor_throttle(throttle_ml[0]);
        command.steering_servo(steering_ml[0]);
