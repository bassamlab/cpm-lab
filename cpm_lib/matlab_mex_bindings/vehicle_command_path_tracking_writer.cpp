...
private:
    ...
    std::shared_ptr<cpm::Writer<VehicleCommandPathTrackingPubSubType>> writer;
public:
    /**
     ...
     * \param inputs Inputs given by the calling Matlab script. Here: The VehicleCommandPathTracking Matlab Object to send. Optional: Matlab Domain ID.
     */
    void operator()(matlab::mex::ArgumentList outputs, matlab::mex::ArgumentList inputs) {   
        ...
        if (!participant || !writer)
        {
            ...
            writer = std::make_shared<cpm::Writer<VehicleCommandPathTrackingPubSubType>>(
    ...
    void checkArguments(matlab::mex::ArgumentList outputs, matlab::mex::ArgumentList inputs) {
        ...
        if (inputs.size() < 1 || inputs.size() > 2)
        {
            matlabPtr->feval(u"error", 0, 
                std::vector<matlab::data::Array>({ factory.createScalar("Wrong input size. Input must be an object of type VehicleCommandPathTracking, followed by an optional domain_id argument (default is 1)!") }));
        }

        //Test for correct class w.r.t. message to send
        std::vector<matlab::data::Array> args{ inputs[0], factory.createCharArray("VehicleCommandPathTracking") };
        matlab::data::TypedArray<bool> result = matlabPtr->feval(u"isa", args);
        if (result[0] != true) {
            matlabPtr->feval(u"error", 0, 
               std::vector<matlab::data::Array>({ factory.createScalar("Input must be an object of type VehicleCommandPathTracking!") }));
        } 
     ...
     */
    VehicleCommandPathTracking get_vehicle_command_from_input(matlab::mex::ArgumentList inputs)
    {
        matlab::data::ArrayFactory factory;

        VehicleCommandPathTracking command;
        ...
        //Get path point data (is a 1xn vector in Matlab)
        std::vector<PathPoint> path_points;
        matlab::data::ObjectArray path_points_ml = matlabPtr->getProperty(object, 0, u"path");
        for (auto i = 0; i < path_points_ml.getDimensions().at(1); ++i) {
            PathPoint path_point;

            Pose2D pose;

            matlab::data::TypedArray<double> x_ml = matlabPtr->getProperty(path_points_ml, i, u"x");
            pose.x(x_ml[0]);

            matlab::data::TypedArray<double> y_ml = matlabPtr->getProperty(path_points_ml, i, u"y");
            pose.y(y_ml[0]);

            matlab::data::TypedArray<double> yaw_ml = matlabPtr->getProperty(path_points_ml, i, u"yaw");
            pose.yaw(yaw_ml[0]);

            path_point.pose(pose);

            matlab::data::TypedArray<double> s_ml = matlabPtr->getProperty(path_points_ml, i, u"s");
            path_point.s(s_ml[0]);
