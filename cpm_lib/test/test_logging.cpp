...
TEST_CASE("Logging") {
    ...
    // Get Stringstream version to check if the Logger treats data like a
    // stringstream (which it should)
    ...
    // Store the current file content of the log file - it should match the
    // actual_content stringstream
    ...
    // Some milliseconds need to pass, else the order of the logs is not
    // guaranteed
    ...
    // Compare thread content (received messages) with desired content (order
    // irrelevant)
    CHECK((std::find(listener_content.begin(), listener_content.end(),
                    actual_content.str()) != listener_content.end() &&
            std::find(listener_content.begin(), listener_content.end(),
                    (second_test + with_more)) != listener_content.end() &&
            std::find(listener_content.begin(), listener_content.end(),
                    third_test) != listener_content.end()));
    CHECK(thread_id == id);

    std::cout << "Testing logging DONE" << std::endl;
}