...
Deploy::Deploy(
    ...
    std::string _absolute_exec_path) : cmd_domain_id(_cmd_domain_id),
...
std::vector<std::string> Deploy::check_for_crashes(bool script_started, bool deploy_distributed, bool has_local_hlc, bool lab_mode_on, bool check_for_recording)
{
    ...
    if (!(deploy_distributed))
    {
        if (script_started)
        {
            if (!session_exists(hlc_session))
                crashed_participants.push_back("HLC");
        }

        std::string middleware_session_name = middleware_session + "_" + std::to_string(default_middleware_domain_id);
        if (!session_exists(middleware_session_name))
            crashed_participants.push_back(middleware_session_name);
    }
    ...
    if (lab_mode_on)
    {
        if (!session_exists(ips_session))
            crashed_participants.push_back("IPS");
        if (!session_exists(basler_session))
            crashed_participants.push_back("Basler LED detection");
    }
    if (check_for_recording)
    {
        if (!session_exists(recording_session))
            crashed_participants.push_back("DDS Recording");
        if (!session_exists(labcam_session))
            crashed_participants.push_back("LabCam");
    }
